...
def on_entry_click(entry, placeholder):
    if entry.get() == placeholder:
        ...
        if placeholder in ["Password", "Conferma Password"]:
            entry.config(show="*")
...
# Funzione per associare i bind senza lambda
def bind_placeholder_events(entry, placeholder):
    def on_focus_in(event):
        on_entry_click(entry, placeholder)
    def on_focus_out(event):
        on_focusout(entry, placeholder)
...
def registration_screen():
    ...
    for field in fields:
        ...
        bind_placeholder_events(entry, field["placeholder"])
    ...
    login_link = tk.Label(root, text="Accedi ora", font=(font_normal[0], font_normal[1], "bold"),
                          fg=BUTTON_COLOR, bg=BG_COLOR, cursor="hand2")
...
def login_screen():
    ...
    tk.Frame(root, width=100, height=100, bg="#f8f8f8", highlightbackground="#ddd",
             highlightthickness=1).place(x=WIDTH/2-50, y=240)
    ...
    tk.Label(root, text="Password dimenticata?", font=font_normal, fg=BUTTON_COLOR,
             bg=BG_COLOR).place(x=390, y=480, anchor="e")

    btn = tk.Button(root, text="ACCEDI", font=font_button, bg=BUTTON_COLOR, fg="white",
                    command=show_empty_screen)
    ...
    register_link = tk.Label(root, text="Registrati", font=(font_normal[0], font_normal[1], "bold"),
                             fg=BUTTON_COLOR, bg=BG_COLOR, cursor="hand2")
